...
class plgSystemVM2_Cart extends JPlugin {
	...
	function onAfterInitialise() {
		...
		//if (!class_exists( 'VmImage' )) require_once(JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_virtuemart'.DS.'helpers'.DS.'image.php');
		...
		if(JRequest::getCmd('option')=='com_virtuemart' && JRequest::getCmd('view')=='cart' && JRequest::getCmd('task')=='viewJS' && JRequest::getCmd('format')=='json') {
			...
			$cart->billTotal = '<span class="flexibleLabel">'.JText::_('COM_VIRTUEMART_CART_OVERVIEW') . '</span><span class="flexibleNumber">' . $cart->totalProduct . '</span>';
